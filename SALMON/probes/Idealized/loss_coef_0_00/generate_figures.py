...
def calculate_magnitude_gain(data, alpha):
    ...
    # Format alpha value for column name
    if alpha == 0.0:
        col_name = 'cos_S_FP@0.00'
    elif alpha == 0.1:
        col_name = 'cos_S_FP@0.10'
    elif alpha == 0.25:
        col_name = 'cos_S_FP@0.25'
    elif alpha == 0.5:
        col_name = 'cos_S_FP@0.50'
    elif alpha == 1.0:
        col_name = 'cos_S_FP@1.00'
    else:
        col_name = f'cos_S_FP@{alpha:.2g}'
    
    cos_S_FP = data[col_name].values if col_name in data.columns else data['cos_S_FP'].values
...
def figure4_safe_mixing_window(data, output_dir):
        ...
        for locus in ['L1_fea', 'L2_fea', 'L3_fea']:
            ...
            for alpha in alpha_values:
                if alpha == 0:
                    ...
                    # Format alpha value for column name
                    if alpha == 0.0:
                        col_name = 'cos_S_task@0.00'
                    elif alpha == 0.1:
                        col_name = 'cos_S_task@0.10'
                    elif alpha == 0.25:
                        col_name = 'cos_S_task@0.25'
                    elif alpha == 0.5:
                        col_name = 'cos_S_task@0.50'
                    elif alpha == 1.0:
                        col_name = 'cos_S_task@1.00'
                    else:
                        col_name = f'cos_S_task@{alpha:.2g}'
                    cos_S_task = locus_data[col_name].values if col_name in locus_data.columns else np.ones(len(locus_data))
            ...
            # Add alpha labels for key points
            for alpha_label in [0.0, 0.1, 0.2, 0.5, 1.0]:
                idx = int(alpha_label * 20)  # Convert to index
                if idx < len(M_values_all):
                    ax.annotate(f'α={alpha_label:.1f}', 
    ...
    ax.set_xlabel(r'$M_i(\alpha)$ (FP32-Parallel Magnitude Gain)')
    ax.set_ylabel(r'$\rho_i(\alpha) = \cos^2(S_i(\alpha), A_i)$ (Curvature Safety)')
    ax.set_title(f'Figure 4: Safe Mixing Window (Epoch {epoch})')
    ax.legend(loc='best')
    ax.set_xlim([0.5, 3.0])
    ...
    ax.text(2.0, 0.55, 'α increase', fontsize=9, color='gray', rotation=-60)
