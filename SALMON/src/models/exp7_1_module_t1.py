    ...
    def training_step(self, batch, batch_idx):
        ...
        for idx, (output, feature) in enumerate(zip(outputs, features)):
            if idx != 3:  # Exclude the teacher index from direct self-comparison
                ...
                if idx > 3:  # Only use adaptation layers for features after the teacher feature
                    loss += torch.dist(self.adaptation_layers[idx - 4](feature), teacher_feature) * self.feature_loss_coefficient

        # Metrics update and logging
        self.train_acc(torch.argmax(teacher_output, dim=1), labels)
