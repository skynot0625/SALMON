...
class SalmonLitModule(LightningModule):
        ...
        for branch_name, gD_dict in gD_split.items():
            ...
            for locus in ['L1_fea', 'L2_fea', 'L3_fea', 'FB']:
                ...
                for alpha in alpha_eval_list:
                    row[f'cos_S_FP@{alpha:.1f}'] = None
                    row[f'descent_gain_task@{alpha:.1f}'] = alpha * beta_task
            ...
            for locus in ['L1_fea', 'L2_fea', 'L3_fea', 'FB']:
                ...
                for alpha in alpha_eval_list:
                    ...
                    rows.append({
                        ...
                        'locus': locus,
                        'descent_ratio': ratio,
                        f'descent_ratio@{alpha:.1f}': ratio,
                        'dot_ad': dot_ad,
            ...
            for locus in ['L1_fea', 'L2_fea', 'L3_fea', 'FB']:
                ...
                for alpha in alpha_eval_list:
                    ...
                    rows.append({
                        'branch': branch,
                        'locus': locus,
                        f'cos_S_task@{alpha:.1f}': cos_S_task,
                        f'dir_deriv_task@{alpha:.1f}': dir_deriv
                    })
        return rows
        ...
        for branch_name, gD_dict in gD_split.items():
            ...
            for locus in ['L1_fea', 'L2_fea', 'L3_fea', 'FB']:
                ...
                for alpha in alpha_eval_list:
                    ...
                    cos_S_FP_values[f'cos_S_FP@{alpha:.1f}'] = cos_S_FP
                ...
                if best_alpha > 0 and norm_A > 1e-8:
                    angle_improve_deg = angle_A_FP_deg - np.arccos(np.clip(cos_S_FP_values[f'cos_S_FP@{best_alpha:.1f}'], -1, 1)) * 180 / np.pi
                else:
                    angle_improve_deg = 0
    ...
    def _compute_alignment_metrics(self, g_analog, g_fp32, gD_split, alpha_eval_list):
        ...
        for branch_name, gD_dict in gD_split.items():
            ...
            for locus in ['L1_fea', 'L2_fea', 'L3_fea', 'FB']:
                ...
                angle_improve_deg = np.arccos(cos_A_FP) - np.arccos(cos_S_FP_values[f'cos_S_FP@{best_alpha:.1f}'])
    ...
    def _create_epoch_plots(self, epoch: int, df: pd.DataFrame, has_fp32: bool):
        ...
        # Save plot
        os.makedirs('./probes/plots', exist_ok=True)
        plot_path = f'./probes/plots/epoch_{epoch}_triple_alignment.png'
        ...
        for L in loci_task:
            for B in branches:
                ...
                if len(s) > 0:
                    ...
                    gain_path = f'./probes/plots/epoch_{epoch}_gain_task_{B}_{L}.png'
        ...
        for L in loci_task:
            for B in branches:
                ...
                if len(s) > 0:
                    ...
                    descent_path = f'./probes/plots/epoch_{epoch}_descent_ratio_{B}_{L}.png'
        ...
        for L in loci_task:
            for B in branches:
                ...
                if len(s) > 0:
                    ...
                    dir_path = f'./probes/plots/epoch_{epoch}_directional_{B}_{L}.png'
    ...
    def _create_epoch_csv_files(self, epoch: int, seed: int, has_fp32: bool):
        ...
        for alpha in self.probe_alpha_eval:
            fieldnames_task.append(f'descent_gain_task@{alpha:.1f}')
            fieldnames_task.append(f'descent_ratio@{alpha:.1f}')  # NEW
            fieldnames_task.append(f'cos_S_task@{alpha:.1f}')  # NEW
            fieldnames_task.append(f'dir_deriv_task@{alpha:.1f}')  # NEW
        ...
        if has_fp32:
            ...
            for alpha in self.probe_alpha_eval:
                fieldnames_fp32.append(f'cos_S_FP@{alpha:.1f}')
