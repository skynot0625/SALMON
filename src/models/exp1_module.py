...
class SalmonLitModule(LightningModule):
    ...
    def configure_optimizers(self):
        # optimizer 설정을 동적으로 로드합니다.
        optimizer_config = self.hparams.optimizer
        optimizer = getattr(torch.optim, optimizer_config['_target_'])(self.parameters(), **{k: v for k, v in optimizer_config.items() if k not in ['_target_', '_partial_']})
        
        # scheduler 설정이 있다면 동적으로 로드합니다.
        if self.hparams.scheduler is not None:
            scheduler_config = self.hparams.scheduler
            scheduler = getattr(torch.optim.lr_scheduler, scheduler_config['_target_'])(optimizer, **{k: v for k, v in scheduler_config.items() if k not in ['_target_', '_partial_']})
            scheduler_config = {
                'scheduler': scheduler,
                'monitor': 'val/loss',  # 예시 목적으로 val/loss를 사용; 실제 사용 시 적절히 조정 필요
                'interval': 'epoch',
                'frequency': 1,
            }
