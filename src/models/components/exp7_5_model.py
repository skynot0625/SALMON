...
class ResNetFeatures(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False, groups=1, 
        ...
        self.inplanes = base_channels  # 기본 채널 설정 업데이트
        self.dilation = 1
        if replace_stride_with_dilation is None:
            self.replace_stride_with_dilation = [False, False, False]
        else:
        ...
        # Creating layers with updated base_channels
...
class IntegratedResNet(nn.Module):
    def __init__(self, architecture="resnet10", num_classes=10, rpu_config=None):
        ...
        self.input_module = create_input_module(in_channels=3, base_channels=128)  # Updated base_channels
        ...
        in_features = 1024 * block_type_1.expansion  # Updated in_features based on the new base_channels
